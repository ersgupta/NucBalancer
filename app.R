...
server <- function(input, output, session) {
	...
	process_data <- function(samples, barcodes){
		...
		shinyjs::html(id = "text", html = paste0("Maximum Runtime (in mins): ", runtime, "<br><br>"), add=TRUE)
		shinyjs::html(id = "text", html = paste0("Sample file: <br>"), add=TRUE)
		shinyjs::html(id = "text", html = print(xtable(samples, include.rownames=F), type="html"), add=T)
		
		shinyjs::html(id = "text", html = paste0("Barcodes file (first 10): <br>"), add=TRUE)
		shinyjs::html(id = "text", html = print(xtable(head(barcodes,10), include.rownames=F), type="html"), add=T)
	...
	if(round(sum(samples$pct)*100) != 100){
	  ...
	  exit(1)
		...
		shinyjs::html(id = "text", html = paste0("Files passed QC!<br>"), add=TRUE)
	  shinyjs::html(id = "text", html = paste0("Finding barcodes...<br>"), add=TRUE)
	}
